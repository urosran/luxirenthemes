...
import FooterWithDeco from '../movie/Footer/FooterWithDeco';
import PageNav from '../movie/PageNav';
import Notification from '../movie/Notification';
// Showcase Setup
import ThemePalette from './ThemePalette';
import BgDefault from './BgDefault';
import { i18n } from '../../i18n';
import appTheme from '../../theme/movieMui';
...
function Movie(props) {
  ...
  const [invertHeader, switchHeader] = useState(false);
  const [direction, changeDir] = useState('ltr');
  const [themeMode, changeMode] = useState(themeType);
  const [color, changeColor] = useState('passion');
  ...
  useEffect(() => {
    ...
    // Check URL to simulate is in detail page
    const url = window.location.href;
    const isDetailPage = url.includes('detail-movie');
    switchHeader(isDetailPage);
  });
